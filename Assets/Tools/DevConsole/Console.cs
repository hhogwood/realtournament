    ...
    {		
		...
		void OnGUI(){
			...
			if (current.type == EventType.KeyDown && current.keyCode == consoleKey)
            {
                ...
                if (Singleton.PauseWhileOpen == true && closed == false)
                {
                    Time.timeScale = 1;
                }

                if (Singleton.UnlockCursor == true && closed == false)
                {
		...
		IEnumerator FadeInOut(bool open)
        {
            ...
            if (Singleton.PauseWhileOpen == true && open == true)
            {            
                Time.timeScale = 0;
            }

            if (Singleton.UnlockCursor == true && open == true)
            {
                Cursor.visible = true;
